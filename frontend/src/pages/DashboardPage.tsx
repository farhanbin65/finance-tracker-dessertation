 ...
 * UI UX Pro Max: Vector icons, shimmer skeletons, accessibility,
 * empty states, desktop max-width, negative savings rate fix
 */
...
// ── Category icon map — Tabler icons (no emoji) ────────────────────
const CAT_ICON: Record<string, string> = {
  ...
  Health: 'ti-heart-rate-monitor', Salary: 'ti-cash',
  Other: 'ti-credit-card',
}

function catIcon(cat: string) {
  return CAT_ICON[cat] || 'ti-credit-card'
}
...
function Shimmer({ height, radius = 14 }: { height: number; radius?: number }) {
  return (
    <div style={{
...
function SectionRow({ title, onSeeAll }: { title: string; onSeeAll: () => void }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between',
      ...
      {/* ✅ button instead of span — keyboard accessible */}
      <button
        onClick={onSeeAll}
        style={{
...
function EmptyState({ icon, message }: { icon: string; message: string }) {
  return (
    <div style={{
      ...
      <i className={`ti ${icon}`} style={{ fontSize: 28, color: 'var(--text-muted)', opacity: 0.5 }} aria-hidden="true" />
...
export default function DashboardPage() {
  ...
  // ✅ Floor savings rate at 0 to avoid showing negative %
  ...
  const avgBudgetUsed = budgets.length > 0
    ? Math.round(budgets.reduce((s, b) => s + b.percentage_used, 0) / budgets.length)
    : 0
  ...
  return (
    // ✅ max-w-2xl so content doesn't stretch on wide desktop
    <div style={{ maxWidth: 720, margin: '0 auto', paddingBottom: 32 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 700, lineHeight: 1.2, color: 'var(--text-primary)' }}>
          {firstName}
        </h1>
      ...
      {summary && (
        <div style={{
          ...
          <p style={{
            ...
            fontWeight: 600, letterSpacing: '.06em', marginBottom: 6,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: '10px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                <i className="ti ti-trending-up" style={{ fontSize: 14, color: 'rgba(255,255,255,0.8)' }} aria-hidden="true" />
                <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.65)',
                  fontWeight: 600, letterSpacing: '.05em', textTransform: 'uppercase' }}>Income</span>
              </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: '10px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                <i className="ti ti-trending-down" style={{ fontSize: 14, color: 'rgba(255,255,255,0.8)' }} aria-hidden="true" />
                <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.65)',
                  fontWeight: 600, letterSpacing: '.05em', textTransform: 'uppercase' }}>Spent</span>
              </div>
              <p style={{ fontSize: 16, fontWeight: 700, color: '#fff' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <p style={{
              ...
              letterSpacing: '.05em', marginBottom: 5, textTransform: 'uppercase',
            }}>FinSight AI</p>
            {aiTipLoading ? (
              <>
                <style>{`@keyframes shimmer{0%{transform:translateX(-100%)}100%{transform:translateX(100%)}}`}</style>
      ...
      <div style={{
        ...
        {budgets.length === 0 ? (
          <EmptyState icon="ti-wallet" message="No budgets set up yet. Tap 'See all' to add one." />
        ) : (
      ...
      <div style={{
        ...
        {transactions.length === 0 ? (
          <EmptyState icon="ti-receipt" message="No transactions yet. Add your first one." />
        ) : (
              ...
              <div key={tx.id} style={{
                ...
                <div style={{
                  ...
                  background: tx.type === 'income'
                    ? 'rgba(34,200,122,0.12)' : 'rgba(255,159,67,0.1)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  <i className={`ti ${catIcon(tx.category)}`}
                     style={{
                       fontSize: 17,
                       color: tx.type === 'income' ? 'var(--green)' : CAT_COLORS[tx.category] || 'var(--orange)',
                     }}
                     aria-hidden="true" />
                </div>
      ...
      <div style={{
        ...
        {goals.length === 0 ? (
          <EmptyState icon="ti-target" message="No goals yet. Start saving towards something!" />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {goals.map(g => {
              const pct = Math.min(g.percentage || 0, 100)
              // ✅ Graceful emoji fallback — vector icon if backend sends ??
              const hasValidEmoji = g.emoji && g.emoji !== '??' && g.emoji.length <= 4
              return (
